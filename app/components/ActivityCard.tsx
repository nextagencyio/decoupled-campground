...
import { DrupalActivity } from '@/lib/types'
        ...
        {(item as any).image?.url ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-20 h-20 bg-primary-200 rounded-full flex items-center justify-center">
              <Mountain className="w-10 h-10 text-primary-500" />
            </div>
          </div>
        )}
        {(item as any).difficulty && (
          <span className="absolute top-3 left-3 bg-accent-600 text-white text-xs font-semibold px-3 py-1 rounded-full">
            {(item as any).difficulty}
          </span>
        )}
      </div>
