...
export default function HeroSection({ homepageContent }: HeroSectionProps) {
  const title = (homepageContent as any)?.heroTitle || (homepageContent as any)?.title || 'Your Home Under the Pines'
  const subtitle = (homepageContent as any)?.heroSubtitle || 'Where adventure meets tranquility'
    ...
    <section className="relative min-h-[80vh] flex items-center overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center pt-32 pb-32">
        <div className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm rounded-full px-4 py-2 mb-8">
          ...
          <span className="text-white/90 text-sm font-medium">Now Accepting Reservations for 2026</span>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <a
            href="/contact"
            className="inline-flex items-center justify-center px-8 py-4 bg-accent-600 text-white rounded-full hover:bg-accent-700 transition-all duration-200 font-semibold text-lg shadow-lg hover:shadow-xl hover:-translate-y-0.5"
          >
            Book a Site
          </a>
          <a
