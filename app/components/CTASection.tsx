...
export default function CTASection({ homepageContent }: CTASectionProps) {
  const title = (homepageContent as any)?.ctaTitle || 'Ready for Your Next Adventure?'
  const description = (homepageContent as any)?.ctaDescription?.processed || ''
  const primaryLabel = (homepageContent as any)?.ctaPrimary || 'Reserve Your Site'
  const secondaryLabel = (homepageContent as any)?.ctaSecondary || 'View Campsite Map'
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center pt-10">
        ...
        {description ? (
          ...
          <p className="text-primary-100 mb-10 max-w-2xl mx-auto text-lg">
            From tent camping under the stars to full-hookup RV sites, rustic cabins, and glamping tents -- find your perfect spot in the pines.
          </p>
        )}
