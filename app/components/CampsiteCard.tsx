...
import { DrupalCampsite } from '@/lib/types'
...
export default function CampsiteCard({ item }: CampsiteCardProps) {
  return (
    <Link
      href={item.path || '#'}
    ...
    >
      <div className="relative h-52 bg-gradient-to-br from-primary-100 to-primary-200">
        ...
        {(item as any).rate && (
          <span className="absolute top-3 left-3 bg-primary-700 text-white text-xs font-semibold px-3 py-1 rounded-full">
            {(item as any).rate}
          </span>
        )}
      </div>
